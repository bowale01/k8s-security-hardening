...
# - 3 private subnets (for EKS worker nodes - no internet access)
# - 3 NAT gateways (one per AZ for high availability)
# - Internet gateway (for outbound traffic)
#
# Why this matters:
# - Worker nodes in private subnets can't be directly attacked from internet
# - NAT gateways provide controlled outbound access only
